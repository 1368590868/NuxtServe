...
export default {
  data() {
    ...
    return {
      form: {
        ...
        logoLocalImageUrl: this.$store.state.imgUrl, // 存储在内容中的logo图片的地址，非七牛地址，由URL.createObjectURL创建的一个地址
        author: "冉林",
        createAt: new Date(),
      },
      uptoken: {
  ...
  methods: {
    ...
    handleArticleLogoSubmit() {
      ...
      // });
      this.$axios.post("/api/article/upload", fd).then(res => {
        console.log(res);
    ...
    onSubmit() {
      //向服务器发送保存的数据
      this.$axios.post("/api/article", this.form).then(res => {
        if (res.data.result === "success") {
